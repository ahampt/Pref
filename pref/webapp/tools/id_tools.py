...
# Return wikipedia url identifier given movie object
def wikipedia_id_from_movie(movie):
	if movie and movie.Title and movie.Year:
		try:
			# Query wikipedia search API
			req = urllib2.Request('http://en.wikipedia.org/w/api.php?format=xml&action=query&list=search&srlimit=10&srsearch='+urllib.quote(movie.Title.encode('ascii', 'ignore'))+'%20'+str(movie.Year)+'%20film')
			...
			if res.getcode() == 200:
				...
				for node in dom.getElementsByTagName('link'):
					if node.getAttribute('title') == 'directors' or node.getAttribute('title') == 'cast':
						...
						res = urllib2.urlopen(req)
						if res.getcode() == 200:
							director_dom = parseString(res.read())
							for person in director_dom.getElementsByTagName('person'):
								if not is_cast:
...
def movie_from_rottentomatoes_input(rottentomatoes_input):
	...
	if rottentomatoes_id:
		try:
			...
			if res.getcode() == 200:
				# Parse json response
				movie_dict = json.loads(res.read())
